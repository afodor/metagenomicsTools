...
public class PivotRDP
{
	public static void main(String[] args) throws Exception
	{
		for( int x=1; x < NewRDPParserFileLine.TAXA_ARRAY.length; x++)
		{
			...
			getMap(ConfigReader.getKylieAgeDir()+ File.separator + "rdp" 
					+ File.separator + "all" + level + ".txt.gz");
			
			File outFile = new File(ConfigReader.getKylieAgeDir()+ File.separator + "rdp" 
					+ File.separator + "all" + level + "asColumns.txt");
			
			PivotOTUs.writeResults(map, outFile.getAbsolutePath());
			
			OtuWrapper wrapper = new OtuWrapper(outFile);
			
			wrapper.writeNormalizedLoggedDataToFile(ConfigReader.getKylieAgeDir()+ File.separator + "rdp" 
					+ File.separator + "all" + level + "asColumnsLogNorm.txt");
		}
	}
