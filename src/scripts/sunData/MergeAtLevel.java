...
public class MergeAtLevel
{
	...
	private static Map<String, Map<String,Integer>> mergeAtLevel(String level ) throws Exception
	{
		...
		File file1 = new File("C:\\sunData\\otu_table_no_singletons_no_chimeras.txt");
		...
		throws Exception
	{
		...
		File file = new File(
				"c:\\sunData\\"+ level + "_merged.txt"	);
		
		BufferedWriter writer = new BufferedWriter(new FileWriter(file));
		
		writer.write("sample\tgroup\tsequenceDepth");
		...
		for(String s : map.keySet())
		{
			...
			{
				numWritten++;
				writer.write(s + "\t" + s.substring(4) + "\t" +  allCount);
