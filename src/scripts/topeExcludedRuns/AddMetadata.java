...
public class AddMetadata
{
	...
	public static void main(String[] args) throws Exception
	{
		for( int x=1; x < NewRDPParserFileLine.TAXA_ARRAY.length; x++)
		{
			...
			OtuWrapper wrapper = new OtuWrapper( ConfigReader.getTopeFeb2016Dir() 
					+ File.separator + "spreadsheetsChooseOne" +
					File.separator + "pivoted_" + 
			NewRDPParserFileLine.TAXA_ARRAY[x] + "asColumnsThreeOrFour.txt");
			
			File logNormalizedFile = new File(	ConfigReader.getTopeFeb2016Dir() 
					+ File.separator + "spreadsheetsChooseOne" +
					File.separator + "pivoted_" + 
			NewRDPParserFileLine.TAXA_ARRAY[x] + "asColumnsLogNormalThreeOrFour.txt");
			
			wrapper.writeNormalizedLoggedDataToFile(logNormalizedFile);
			
			File outFile = new File( ConfigReader.getTopeFeb2016Dir() 
					+ File.separator + "spreadsheetsChooseOne" +
					File.separator + "pivoted_" + 
					NewRDPParserFileLine.TAXA_ARRAY[x] + "asColumnsLogNormalPlusMetadataThreeOrFour.txt");
			
			addMetadata(wrapper, logNormalizedFile, outFile,false);
			
			/*
			logNormalizedFile = new File(ConfigReader.getTopeFeb2016Dir() 
					+ File.separator + "spreadsheets" +
					File.separator + "mds_"+ NewRDPParserFileLine.TAXA_ARRAY[x] +  ".txt" );
			
			outFile = new File( ConfigReader.getTopeFeb2016Dir() 
					+ File.separator + "spreadsheets" +
					File.separator + "pivoted_" + 
