...
public class PivotRDP
{
	public static void main(String[] args) throws Exception
	{
		for( int x=1; x < NewRDPParserFileLine.TAXA_ARRAY.length; x++)
		{
			...
			getMap(ConfigReader.getMbqcDir()+ File.separator + "rdpAnalysis" 
					+ File.separator + "sparse_" + NewRDPParserFileLine.TAXA_ARRAY[x] 
							+ "_threeCol.txt.gz" );
			
			File outFile = new File(ConfigReader.getMbqcDir()+ File.separator + "rdpAnalysis" 
					+ File.separator + "sparse_" + 
			...
			wrapper.writeNormalizedLoggedDataToFile(ConfigReader.getMbqcDir()+ File.separator + "rdpAnalysis" 
					+ File.separator + "sparse_" + NewRDPParserFileLine.TAXA_ARRAY[x]  
